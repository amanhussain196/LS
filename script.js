    ...
    if (uploadTrigger && imageInput && artCanvas) {
        ...
        if (btnGoReader && readerView) {
            btnGoReader.addEventListener('click', () => {
                // LOCK CHECK: Only unlocked status can go to reader
                if (!isDownloadUnlocked) {
                    if (redeemModal) {
                        redeemModal.classList.remove('hidden');
                ...
                // Auto-load sequence if available from generation or gallery
                if (generatorEngine && generatorEngine.sequence && generatorEngine.sequence.length > 0) {
                    // Convert 0-based sequence (internal) to 1-based CSV (human/reader format)
                    const seqText = generatorEngine.sequence.map(n => n + 1).join(', ');
                    loadReaderSequence(seqText);
                }
            });
        }
